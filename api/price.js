...
export default async function handler(req, res) {
  try {
    ...
    if (cached && Date.now() - cached.ts < 2500) {
      return res.status(200).json({ cached: true, price: cached.data });
    }

    const token = await getAccessToken();
    const price = await getPrice(token, code);
    cache.set(code, { ts: Date.now(), data: price });
    res.status(200).json({ cached: false, price });
  } catch (e) {
...
async function getAccessToken() {
  ...
  if (!appkey || !appsecret) throw new Error("KIS_APP_KEY / KIS_APP_SECRET 환경변수가 필요합니다.");
  ...
  tokenCache = { token: j.access_token, expiresAt: Date.now() + 23 * 60 * 60 * 1000 };
...
async function getPrice(token, code) {
  ...
  const j = await r.json();
  if (!j.output) throw new Error(j.msg1 || j.message || JSON.stringify(j));
  return {
    code,
